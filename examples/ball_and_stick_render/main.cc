...
int main(int argc, char **argv) {
  ...
  const unsigned int nbonds = bondLines.size() / (3 * 2);
  ...
  lsglBufferDataPointer(GL_ARRAY_BUFFER, nbonds * sizeof(float) * 3 * 2, &bondLines.at(0), GL_STATIC_DRAW);
  ...
  // 2.1 draw atoms
  //glBindBuffer(GL_ARRAY_BUFFER, ptmat);
  //glVertexAttribPointer(matPos, 1, GL_FLOAT, GL_FALSE, sizeof(int), (void*)0);
  //glEnableVertexAttribArray(matPos);
  ...
  glDrawArrays(GL_LINES, 0, 2*nbonds);
  assert(glGetError() == GL_NO_ERROR);
