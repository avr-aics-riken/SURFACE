...
#define MAX_TREE_DEPTH_32BIT   (22)   // FYI, log2(1G/16) ~= 25.897, log2(1G/32) ~= 21
...
static void ContributeBinBuffer(BinBuffer *bins, // [out]
                                ...
                                const Tetrahedron *tetras, unsigned int *indices,
...
template<typename T>
void MakeLeaf32(TetraNode &leaf, const T *vertices, const uint32_t *faces, real3 &bmin,
                real3 &bmax, const std::vector<IndexKey30> &keys,
                uint32_t leftIndex, uint32_t rightIndex) {

  // 1. Compute leaf AABB
  ComputeBoundingBox30(bmin, bmax, vertices, faces, keys, leftIndex, rightIndex);
...
size_t BuildTreeRecursive32(std::vector<TetraNode> &nodes, real3 &bmin,
                            ...
                            const T *vertices, const uint32_t *faces, uint32_t rootIndex,
                            uint32_t leftIndex, uint32_t rightIndex,
  ...
  size_t rightChildIndex = BuildTreeRecursive32(
      nodes, rightBMin, rightBMax, keys, nodeInfos, vertices, faces, midIndex + 1,
...
size_t BuildTreeRecursive32OMP(std::vector<TetraNode> &nodes, real3 &bmin,
  ...
  if (depth > 6) {
    ...
    leftChildIndex = BuildTreeRecursive32(
        sub_nodes, leftBMin, leftBMax, keys, nodeInfos, vertices, faces, midIndex,
        leftIndex, midIndex, isLeftLeaf, depth + 1);

    rightChildIndex = BuildTreeRecursive32(
        sub_nodes, rightBMin, rightBMax, keys, nodeInfos, vertices, faces, midIndex + 1,
        midIndex + 1, rightIndex, isRightLeaf, depth + 1);
    ...
    { 
      ...
      for (size_t i = 0; i < sub_nodes.size(); i++) {
        if (sub_nodes[i].flag == 0) { // branch node
          //printf("sub[%d] = %d, %d\n", i, sub_nodes[i].data[0], sub_nodes[i].data[1]);
          ...
          //printf("-> offt = %d, sublen = %d, sub[%d] = %d, %d\n", offset, sub_nodes.size(), i, sub_nodes[i].data[0], sub_nodes[i].data[1]);
        }
      }
    ...
    #pragma omp task shared(leftChildIndex, rightChildIndex, nodes, leftBMin, leftBMax, keys, nodeInfos, vertices, faces) firstprivate(midIndex) if (depth < 10)
    leftChildIndex = BuildTreeRecursive32OMP(
        nodes, leftBMin, leftBMax, keys, nodeInfos, vertices, faces, midIndex,
        leftIndex, midIndex, isLeftLeaf, depth + 1);


    #pragma omp task shared(leftIndex, rightChildIndex, nodes, rightBMin, rightBMax, keys, nodeInfos, vertices, faces) firstprivate(midIndex) if (depth < 10)
...
size_t TetraAccel::BuildTree(const Tetrahedron *tetras, unsigned int leftIdx,
  ...
  for (int axisTry = 0; axisTry < 1; axisTry++) {
    ...
    if (tetras->isDoublePrecisionPos) {
      mid =
          std::partition(begin, end, SAHPredD(cutAxis, cutPos[cutAxis], tetras));
    } else {
      mid = std::partition(begin, end, SAHPred(cutAxis, cutPos[cutAxis], tetras));
    }
...
bool TetraAccel::Build(const Tetrahedron *tetras, const TetraBuildOptions &options) {
  options_ = options;
...
bool TetraAccel::Build32(const Tetrahedron *tetras, const TetraBuildOptions &options) {
...

  {
...
#ifdef _OPENMP
    ComputeBoundingBoxOMP(bmin, bmax, tetras->vertices, tetras->faces, &indices_.at(0), 0, n);
#else
    ComputeBoundingBox(bmin, bmax, tetras->vertices, tetras->faces, &indices_.at(0), 0, n);
#endif
...

    {
      ...
      if (tetras->isDoublePrecisionPos) {
        ...
        CalculateMortonCodesTriangleFloat30(&codes.at(0), tetras->vertices, tetras->faces, bmin, bmax, 0, n);
      }
      t.end();
...
#pragma omp barrier
      ...
      //for (size_t i = 0; i < n-1; i++) {
        ...
        //printf("[%08d] i = %010d, c = %010d(%s)\n", i, keys[i].index, keys[i].code, b.c_str());
...
#pragma omp parallel for schedule(dynamic)
...

    {
      ...
      #pragma omp parallel shared(leftChildIndex, rightChildIndex, keys, nodeInfos)
      {
        #pragma omp single
        {
          leftChildIndex = BuildTreeRecursive32OMP(nodes_, leftBMin, leftBMax, keys, nodeInfos,
                                   tetras->vertices, tetras->faces, midIndex, 0, midIndex, isLeftLeaf, 0);
        }

        #pragma omp single
        {
          rightChildIndex = BuildTreeRecursive32OMP(nodes_, rightBMin, rightBMax, keys, nodeInfos, tetras->vertices, tetras->faces,
      ...
      size_t leftChildIndex =
          BuildTreeRecursive32(nodes_, leftBMin, leftBMax, keys, nodeInfos,
                               tetras->vertices, tetras->faces, midIndex, 0, midIndex, isLeftLeaf, 0);
      size_t rightChildIndex = BuildTreeRecursive32(
          nodes_, rightBMin, rightBMax, keys, nodeInfos, tetras->vertices, tetras->faces,
          midIndex + 1, midIndex + 1, n - 1, isRightLeaf, 0);
...
bool TestLeafNode(Intersection &isect, // [inout]
                  const TetraNode &node, const std::vector<unsigned int> &indices,
...
void BuildIntersection(Intersection &isect, const Tetrahedron *tetras, Ray &ray) {
  // face index
